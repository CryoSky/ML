...
class LogisticRegressor:
    ...
    def loss(self, *args):
        ...
        ##########################################################################
        J = 1.0 / m * sum([-y[i] * np.log(utils.sigmoid(theta.dot(X[i]))) - (1 - y[i]) * np.log(1.0 - utils.sigmoid(theta.dot(X[i]))) for i in xrange(m)])
    ...
    def predict(self, X):
        ...
        y_pred = np.around(utils.sigmoid(self.theta.T.dot(X.T)))
        print y_pred
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
...
class RegLogisticRegressor:
    ...
    def loss(self, *args):
        ...
        # TODO: 1-2 lines of code expected                                       #
        ##########################################################################
        J = 1. / m * sum([-y[i] * np.log(utils.sigmoid(theta.dot(X[i]))) - (1 - y[i]) * np.log(1 -utils.sigmoid(np.dot(X[i],theta))) for i in xrange(m)])
    ...
    def grad_loss(self, *args):
        ...
        for j in xrange(dim):
            if j>=1:
                grad[j] = 1. / m * sum([(utils.sigmoid(theta.dot(X[i])) - y[i]) * X[i][j] for i in xrange(m)])+1. * reg / m * theta[j]
            else:
