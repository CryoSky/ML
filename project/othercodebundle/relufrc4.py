...
h_conv12 = relu(conv2d(h_conv1, W_conv12) + b_conv12)
h_conv121=tf.nn.lrn(h_conv12)
#h_pool2 = max_pool_2x2(h_conv2)

W_conv13 = weight_variable([3, 3, 32, 64])
b_conv13 = bias_variable([64])
h_conv13 = relu(conv2d(h_conv121, W_conv13) + b_conv13)
...
h_conv3 = relu(conv2d(h_pool23, W_conv3) + b_conv3)
h_conv31=tf.nn.lrn(h_conv3)

W_conv32 = weight_variable([3, 3, 256, 256])
b_conv32 = bias_variable([256])
h_conv32 = relu(conv2d(h_conv3, W_conv32) + b_conv32)
h_conv321=tf.nn.lrn(h_conv32)

W_conv33 = weight_variable([3, 3, 256, 512])
b_conv33 = bias_variable([512])
h_conv33 = relu(conv2d(h_conv32, W_conv33) + b_conv33)

h_conv331=tf.nn.lrn(h_conv33)
h_pool3 = frac_pool(h_conv331)[0]
...
W_fc1 = weight_variable([100 * 512, hidden])
b_fc1 = bias_variable([hidden])
h_pool2_flat = tf.reshape(h_pool3, [-1, 100*512])
...
for j in range(10):
    for i in range(int(26040/40)):
        pred = sess.run(y_conv, feed_dict={x: testdatax[40*i:40*i+40], keep_prob: 1})
        print(pred.shape)
