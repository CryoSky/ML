...
extra=1;
recover=0;
ep=3 # maybe this is too large
epex=15 #10 i enough but maybe 15 is better
hidden=512
print ("loading bbb")
...
h_pool7 = max_pool_2x2(h_conv5)

W_fc1 = weight_variable([4*4 * 256, 512])
b_fc1 = bias_variable([512])
h_pool2_flat = tf.reshape(h_pool7, [-1, 4*4*256])
h_fc1 = relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)
keep_prob = tf.placeholder(tf.float32)
h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob)

W_fc11 = weight_variable([512, 512])
b_fc11 = bias_variable([512])
h_pool12_flat = tf.reshape(h_fc1_drop, [-1, 512])
h_fc11 = relu(tf.matmul(h_pool12_flat, W_fc11) + b_fc11)

h_fc1_drop1 = tf.nn.dropout(h_fc11, keep_prob)

W_fc2 = weight_variable([512, 10])
b_fc2 = bias_variable([10])
y_conv = tf.matmul(h_fc1_drop1, W_fc2) + b_fc2
...
for j in range(int(ep)):
  print(j)
  for i in range(int(73200*2/batchsize)):
