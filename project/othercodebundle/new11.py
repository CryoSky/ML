...
for i in range(len(bbb)):
    ...
    #print (np.max(bbb[i]))
    bbb[i]=(bbb[i]-np.min(bbb[i]))/(np.max(bbb[i])-np.min(bbb[i]))
...
for i in range(len(testdatax)):
    ...
    testdatax[i] = (testdatax[i] - np.min(testdatax[i])) / (np.max(testdatax[i]) - np.min(testdatax[i]))
...
h_pool3=tf.nn.lrn(h_pool3)


W_conv4 = weight_variable([3, 3, 192, 192])   #89 as 92
b_conv4 = bias_variable([192])
h_conv4 = tf.nn.relu(conv2d(h_pool3, W_conv4) + b_conv4)


W_conv5 = weight_variable([3, 3, 192, 192]) #91 as 94  #95 97 98
b_conv5 = bias_variable([192])
h_conv5 = tf.nn.relu(conv2d(h_conv4, W_conv5) + b_conv5)
gp=globalpool(h_conv5)


W_fc1 = weight_variable([ 192, 192])
b_fc1 = bias_variable([192])
h_pool2_flat = tf.reshape(gp, [-1, 192])
...
train_step = tf.train.AdamOptimizer(1e-4).minimize(cross_entropy)#1e-4 changed to 1e-3
