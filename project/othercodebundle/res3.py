...
extra=1;
recover=0;
ep=10 # maybe this is too large
epex=10 #10 i enough but maybe 15 is better
hidden=512
print ("loading bbb")
...
# h_pool2 = max_pool_2x2(h_conv2)
W_conv22 = weight_variable([3, 3, 48, 48])
b_conv22 = bias_variable([48])
h_conv22 = tf.nn.relu(conv2d(h_conv21, W_conv22) + b_conv22)
h_conv221=tf.nn.lrn(h_conv22)

W_conv23 = weight_variable([3, 3, 48, 48])
b_conv23 = bias_variable([48])
...
W_conv32 = weight_variable([3, 3, 64, 64])
b_conv32 = bias_variable([64])
h_conv32 = tf.nn.relu(conv2d(h_conv31, W_conv32) + b_conv32)
h_conv321=tf.nn.lrn(h_conv32)

W_conv33 = weight_variable([3, 3, 64, 64])
b_conv33 = bias_variable([64])
h_conv33 = tf.nn.relu(conv2d(h_conv321, W_conv33) + b_conv33)
h_conv331=tf.nn.lrn(h_conv33)
h_poo321 = frac_pool(h_conv331+h_conv31)[0]

W_conv4 = weight_variable([3, 3, 64, 128])   #89 as 92
b_conv4 = bias_variable([128])
h_conv4 = tf.nn.relu(conv2d(h_poo321, W_conv4) + b_conv4)
h_conv41=tf.nn.lrn(h_conv4)
#h_pool4 = max_pool_2x2(h_conv41)

W_conv42 = weight_variable([3, 3, 128, 128])   #89 as 92
b_conv42 = bias_variable([128])
h_conv42 = tf.nn.relu(conv2d(h_conv41, W_conv42) + b_conv42)
h_conv412=tf.nn.lrn(h_conv42)

W_conv43 = weight_variable([3, 3, 128, 128])   #89 as 92
b_conv43 = bias_variable([128])
h_conv43 = tf.nn.relu(conv2d(h_conv412, W_conv43) + b_conv43)
h_conv413=tf.nn.lrn(h_conv43)
h_poo51 = frac_pool(h_conv413)[0]

W_conv5 = weight_variable([3, 3, 128, 256]) #91 as 94  #95 97 98
b_conv5 = bias_variable([256])
h_conv5 = tf.nn.relu(conv2d(h_poo51, W_conv5) + b_conv5)
h_conv51=tf.nn.lrn(h_conv5)

W_conv6 = weight_variable([3, 3, 256, 256]) #91 as 94  #95 97 98
b_conv6 = bias_variable([256])
h_conv6 = tf.nn.relu(conv2d(h_conv51, W_conv6) + b_conv6)
h_conv61=tf.nn.lrn(h_conv6)
...
h_pool2_flat = tf.reshape(h_conv61, [-1, 49*256])
...
saver.restore(sess, "codebackup/res4.ckpt")
...
for j in range(int(ep)):
  ...
  #batch = mnist.train.next_batch(50)
    ...
    train_step.run(feed_dict={x: bbb[batchsize * i:batchsize * i + batchsize], y_: labley[batchsize * i:batchsize * i + batchsize], keep_prob: 1})#0.5 changed to 1
...
saver.save(sess, "codebackup/res4.ckpt")
print('saved')
