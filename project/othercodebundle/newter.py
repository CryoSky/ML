...
if extra==1:
    ...
    f = open('secondtxtra.txt', 'rb')
    exdata2 = pickle.load(f)
    f.close()
    bbb=np.vstack((bbb, exdata2))
    f = open('extralablefef2.txt', 'rb')
    exdatala2 = pickle.load(f)
    f.close()
    print(exdatala.shape)
    labley = np.vstack((labley, exdatala2))
...
#
# W_conv3 = weight_variable([5, 5, 64, 200])
...
train_step = tf.train.AdamOptimizer(5e-4).minimize(cross_entropy)#1e-4 changed to 1e-3
...
for j in range(int(ep)):
  print(j)
  for i in range((700+1000*extra)*repeat):
