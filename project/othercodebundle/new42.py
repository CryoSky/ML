...
W_fc1 = weight_variable([4 * 4 * 256, hidden])
b_fc1 = bias_variable([hidden])
h_pool2_flat = tf.reshape(h_conv5, [-1, 4*4*256])
...
global_step = tf.Variable(0, trainable=False)
boundaries = [int(70000/batchsize*18)]
values = [1.5e-4,1e-4]
...
train_step = tf.train.AdamOptimizer(1.5e-4).minimize(cross_entropy)#1e-4 changed to 1e-3#1e-4 changed to 1e-3
...
for j in range(int(ep)):
  print(j)
  avgtrainsult=[]
  for i in range(int(72000/batchsize)):
