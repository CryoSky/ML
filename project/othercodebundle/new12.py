...
W_fc1 = weight_variable([5 * 5 * 256, 256])
b_fc1 = bias_variable([256])
h_pool2_flat = tf.reshape(h_conv5, [-1, 5*5*256])
...
# starter_learning_rate = 1.5e-4
# learning_rate = tf.train.exponential_decay(starter_learning_rate, global_step,
#                                        72000/40*ep, 0.4, staircase=True)
...
global_step = tf.Variable(0, trainable=False)
boundaries = [int(70000/batchsize*4)]
values = [1.5e-4,1e-4]
...
for j in range(int(ep)):
  print(j)
  avgtrainsult=[]
  for i in range(int(70000/batchsize)):
  #batch = mnist.train.next_batch(50)

    if i%(10000/batchsize)== 0:
      train_accuracy = accuracy.eval(feed_dict={
          x:bbb[batchsize * i:batchsize * i + batchsize], y_: labley[batchsize * i:batchsize * i + batchsize], keep_prob: 1})

      print("step %d, training accuracy %g"%(i, train_accuracy))
      avgtrainsult.append(train_accuracy)
  ...
  trainsult.append(np.average(avgtrainsult))
  b=[]
  for i in range(int(2000/batchsize)):

    a=accuracy.eval(feed_dict={
        x: bbb[70000+batchsize * i:70000+batchsize * i+batchsize], y_: labley[70000+batchsize * i:70000+batchsize * i+batchsize], keep_prob: 1});
    b.append(a)

  valiresult.append(np.mean(b))
  print(np.mean(b))
...
if extra==1:
    ...
    a=accuracy.eval(feed_dict={
    x: bbb[startn:startn+dn], y_: labley[startn:startn+dn], keep_prob: 1});
    print(a)
...
line_up=plt.plot(valiresult, label='vali')
line_down=plt.plot(trainsult, label='train')
