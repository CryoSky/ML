...
h_conv5 = relu(conv2d(h_pool4, W_conv5) + b_conv5)
#h_conv51=tf.nn.lrn(h_conv5)

W_conv6 = weight_variable([3, 3, 256, 256]) #91 as 94  #95 97 98
b_conv6 = bias_variable([256])
h_conv6 = relu(conv2d(h_conv5, W_conv6) + b_conv6)


W_conv7 = weight_variable([3, 3, 256, 512]) #91 as 94  #95 97 98
b_conv7 = bias_variable([512])
h_conv7 = relu(conv2d(h_conv6, W_conv7) + b_conv7)
h_pool7 = max_pool_2x2(h_conv7)

W_fc1 = weight_variable([4*4 * 512, hidden])
b_fc1 = bias_variable([hidden])
h_pool2_flat = tf.reshape(h_pool7, [-1, 4*4*512])
...
global_step = tf.Variable(int(73000*2/batchsize*3+100000/batchsize*8), trainable=False)
boundaries = [int(73000*2/batchsize*3+100000/batchsize*0),int(73000*2/batchsize*3+100000/batchsize*1),int(73000*2/batchsize*3+100000/batchsize*3),int(73000*2/batchsize*3+100000/batchsize*6),int(73000*2/batchsize*3+100000/batchsize*8)]
values = [1.5e-4,1.3e-4,1e-4,0.8e-4,0.4e-4,0.2e-4] #i changed it form 0.4e-4 to 1.5e-4
...
if recover == 1:
    with tf.Session() as session:
        saver.restore(session, "codebackup/model.ckpt")
        print("loaded")
...
for j in range(int(ep)):
  print(j)
  for i in range(int(400*2/batchsize)):
...
if recover!=1:
    save_path = saver.save(sess, "codebackup/model.ckpt")
    print("saved")
else:
    save_path = saver.save(sess, "codebackup/model2.ckpt")
...
happy=0;
plotnumber=[0 for i in range(100)]
ll=0;
for i in range(100):
